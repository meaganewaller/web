...
export const GET = async (_req: NextRequest, { params }: { params: { type: string; slug: string } }) => {
  try {
...
export const POST = async (req: NextRequest, { params }: { params: { type: string; slug: string } }) => {
  try {
    const { type, slug } = params
