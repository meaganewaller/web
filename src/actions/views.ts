...
export const countContentViews = async ({ type, slug }: CountContentViewsParams): Promise<number> => {
...
export const countUserViews = async ({ type, slug, sessionId }: CountUserViewsParams): Promise<number> => {
  const [data, error] = await fetchData<ViewResponse[]>(
...
export const createView = async ({ type, slug, sessionId }: CreateViewParams): Promise<void> => {
  const data = {
    view: {
      viewable_slug: slug,
      viewable_type: type,
