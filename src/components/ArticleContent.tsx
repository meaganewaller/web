...
import { Children, createElement, forwardRef, memo, useState, ReactNode } from 'react'
      ...
      <ReactMarkdown
        ...
        components={{
          h1: ({ node, children, ...props }) => {
            return (
              <h1 {...props} style={{ marginTop: '2rem' }} className="pt-8 font-monoItalic">
                {children}
              </h1>
            )
          },
          h2: ({ node, children, ...props }) => {
            return (
              <h2 className="pt-5 font-monoItalic" {...props} id={getHeadingId(children)} style={{ marginTop: '2rem' }}>
                {children}
              </h2>
            )
          },
          h3: ({ node, children, ...props }) => {
            return (
              <h3 {...props} style={{ marginTop: '2rem' }} className="pt-4 font-monoItalic">
                {children}
              </h3>
            )
          },
...
const CodeBlock = ({ className = 'not-prose', inline = false, children }: CodeBlockProps) => {
  ...
  if (inline || !language)
    return <code className="p-1 m-1 text-lg text-pink-700 bg-pink-200 rounded-md font-monoItalic">{children}</code>
  return (
    <div className="my-4">
